    ...
    result = useAntdTable<Data, Params>(
      async ({ current, pageSize, sorter = {}, filters = {} }) => {
        ...
        const format = (res: OCPListData<Item>) => {
          ...
          const { page: { total_elements = 0, totalElements = 0 } = {}, contents = [] } =
...
export function getYESorNOLabel(value: 'YES' | 'NO') {
  return (find(YES_OR_NO_LIST, item => item.value === value) || {}).label;
}
...
// 判断两个日期是否是同一天
export function getFormateForTimes(times: string[]) {
  const momentArr = times?.map(time => moment(time));
  const minTime = moment.min(momentArr);
  const maxTime = moment.max(momentArr);
  if (!minTime.isSame(maxTime, 'year')) {
    return 'YYYY-MM-DD HH:mm:ss';
  }
  if (
    minTime.isSame(maxTime, 'year') &&
    minTime.isSame(maxTime, 'month') &&
    minTime.isSame(maxTime, 'day')
  ) {
    return 'HH:mm:ss';
  }
