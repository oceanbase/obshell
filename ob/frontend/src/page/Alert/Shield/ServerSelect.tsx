...
export default function ServerSelect({ onChange, value = {} }: ServerSelectProps) {
  ...
  const getServerDefaultOptions = () => {
    return (getSelectList(clusterList, 'observer') as Alert.ServersList[]).map(cluster => ({
  ...
  const getServersOfClusterOptions = (selectedServers: string[]) => {
    ...
    if (res?.length) {
      res.splice(0, 0, {
        value: 'allServers',
        label: formatMessage({
          id: 'OBShell.Alert.Shield.ServerSelect.AllHosts',
          defaultMessage: '全部主机',
        }),
      });
    }
  ...
  const getClusterFromServersOptions = (selectServers: string[]) => {
    const list = getSelectList(clusterList, 'obcluster', undefined, undefined, selectServers);
  ...
  useUpdateEffect(() => {
    ...
    if (!value.observer?.length) {
      ...
      if (value.observer.includes('allServers')) {
        ...
        const newClusterOptions = getClusterFromServersOptions(value.observer) as OptionsType[];
    ...
    <Row gutter={8}>
      <Col span={8}>
        <Select
          ...
          placeholder={formatMessage({
            id: 'OBShell.Alert.Shield.ServerSelect.PleaseSelectACluster',
      ...
      <Col span={16}>
        <Select
          ...
          allowClear
          placeholder={formatMessage({
            id: 'OBShell.Alert.Shield.ServerSelect.PleaseSelectHost',
