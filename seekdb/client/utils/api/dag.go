...
func callApiHelper(sendRequest func(uri string, param interface{}, res interface{}) error, uri string, param interface{}) (*task.DagDetailDTO, error) {
	...
	stdio.Printf("You can view the task details by '%s/obshell task show -i %s --port $obshell_port -d --seekdb'.", path.BinDir(), res.GenericID)
	return res, nil
}
...
func GetAgentLastMaintenanceDag() (dag *task.DagDetailDTO, err error) {
	...
	if ocsErr, ok := err.(*http.ApiError); ok && ocsErr.ErrCode == errors.ErrTaskNotFoundWithReason.Code {
		return nil, nil
	}
	return dag, nil
}
