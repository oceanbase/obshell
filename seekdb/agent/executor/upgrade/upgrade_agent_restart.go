...
func (t *RestartAgentTask) getParams() (err error) {
	if err = t.GetContext().GetAgentDataByAgentKeyWithValue(meta.OCS_AGENT.String(), DATA_BACKUP_DIR, &t.backupDir); err != nil {
		return err
	}
	return nil
}
...
func (t *RestartAgentTask) restartAgent() (err error) {
	cmd := fmt.Sprintf("%s admin restart --port %d --pid %d --base-dir %s", path.ObshellBinPath(),
		meta.OCS_AGENT.GetPort(), global.Pid, global.HomePath)
	t.ExecuteLogf("cmd is %s", cmd)
	command := exec.Command(path.ObshellBinPath(), "admin", "restart", "--port", fmt.Sprint(meta.OCS_AGENT.GetPort()), "--pid", fmt.Sprint(global.Pid), "--base-dir", global.HomePath)
