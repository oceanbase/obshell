...
func (t *BackupAgentForUpgradeTask) BackupAgentForUpgrade() error {
	...
	if err := system.CopyFile(path.ObshellBinPath(), filepath.Join(t.backupDir, constant.PROC_OBSHELL)); err != nil {
		return errors.Wrap(err, "failed to copy obshell binary")
	}
	return nil
}
