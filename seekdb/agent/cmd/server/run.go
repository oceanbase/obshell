...
import (
	...
	"github.com/oceanbase/obshell/seekdb/agent/engine"
...
func (a *Agent) run() (err error) {
	...
	if err := a.startConnenctModule(); err != nil {
		return errors.Wrap(err, "start oceanbase connect module failed")
	}
...
func waitDbConnectInit() {
	...
	for {
		if _, err = oceanbase.GetInstance(); err == nil {
			log.Info("get ob connection success")
			return
		}
		...
		log.Info("get ob connection failed")
