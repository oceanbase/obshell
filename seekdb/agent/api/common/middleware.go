...
import (
	...
	ocshttp "github.com/oceanbase/obshell/seekdb/agent/lib/http"
	"github.com/oceanbase/obshell/seekdb/agent/lib/path"
	"github.com/oceanbase/obshell/seekdb/agent/lib/process"
	"github.com/oceanbase/obshell/seekdb/agent/lib/trace"
...
func HeaderDecrypt(skipRoutes ...string) func(*gin.Context) {
	return func(c *gin.Context) {
		...
		var err error
		if c.Request.Header.Get(constant.OCS_HEADER) == "" {
			c.Next()
			return
		}
...
func BodyDecrypt(skipRoutes ...string) func(*gin.Context) {
	return func(c *gin.Context) {
		...
		headerByte, _ := c.Get(constant.OCS_HEADER)
		if headerByte == nil {
			c.Next()
			return
		}
