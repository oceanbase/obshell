...
import { Select, Radio, Button, Form, message, Modal } from '@oceanbase/design';
...
const AddDatabaseDrawer: React.FC<AddDatabaseDrawerProps> = ({
  ...
  const defaultCharset = database?.charset || 'utf8mb4';
  ...
  const defaultCollation =
    database?.collation || defaultCollations.find(item => item.isDefault === true)?.name;
