...
const SetParameterEditableProTable: React.FC<SetParameterEditableProTableProps> = ({
  ...
  const { run, loading } = useRequest(obclusterParameters, {
    manual: true,
    onSuccess: res => {
      if (res.successful) {
        let parameterData = [];
        if (type === 'cluster') {
          ...
          const tenantParameterList =
            res?.data?.contents?.filter(item => item.scope === 'TENANT') || [];

          // 创建租户，租户参数模板限制只读参数
          parameterData = tenantParameterList.filter(item => item.edit_level !== 'READONLY');
        }
        if (parameterData.length !== 0) {
          // 编辑、复制
          if (drawerType !== 'NEW' && template) {
              ...
              ?.map(item => {
                ...
                return (
                  <Option
                    ...
                    value={item?.name}
                    // 用于筛选
                    optionFilter={`${item?.name}${oracleLabel}${item.info}`}
                    // 用于选中后填充
                    optionLabel={item?.name}
                  >
                    <span>{oracleLabel ? `${item?.name} (${oracleLabel})` : item?.name}</span>
                    <Typography.Text ellipsis={{ tooltip: item?.info }} style={{ maxWidth: 200 }}>
                      <span>{item?.info}</span>
                    </Typography.Text>
                  </Option>
                );
  ...
  if (tenantMode === 'MYSQL' && drawerType === 'NEW_TENANT' && setCompatibleOracleAlret) {
  ...
  return (
    <>
      ...
      {compatibleOracle && !tenantMode && drawerType !== 'NEW_TENANT' && (
        <Alert
          message={formatMessage({
