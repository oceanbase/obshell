...
export function getSimpleComponentByClusterParameterValue(
  ...
  return value && value.is_single_value ? (
    <span>
      {parameterType === 'TENANT'
        ? formatMessage(
            {
...
export function getDetailComponentByParameterValue(
  value: API.ClusterParameter | null,
  historyType?: string
) {
  const { scope: parameterType } = value || {};
  let serverValues, tenantValues;

  serverValues = value?.ob_parameters;
  tenantValues = value?.tenant_value;

  const zones = Object.entries(groupBy(serverValues, item => item.zone)).map(([key, value]) => {
    return {
      name: key,
      servers: value,
    };
