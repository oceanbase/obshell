  ...
  return (
      ...
      onOk={() => {
        validateFields().then(({ fileName }) => {
          ...
          Modal.confirm({
            ...
            onOk: () => {
              if (pkg) {
                runAsync({ version: pkg?.version, release: pkg.release_distribution });
              }
            },
          });
        });
    ...
    >
      <Form form={form} preserve={false} hideRequiredMark={true} className="form-with-small-margin">
        <Form.Item label={'已安装版本'}>{data?.data?.version}</Form.Item>
        {/* <Form.Item label={'硬件架构'}>{agentData?.identity}</Form.Item> */}
        <Form.Item
          label={'OCP Agent 版本'}
          name="fileName"
          rules={[
