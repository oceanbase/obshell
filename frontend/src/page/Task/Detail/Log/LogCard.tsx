...
const LogCard: React.FC<LogCardProps> = ({
  ...
  const { logNodeList, logList, failedLogList } = getTaskLog(logData || []);
  ...
  // const displayLog =
  //   logNodeList.length > 0 ? logList[logNodeList.indexOf(logNode)] : logList.join('\n');

  const displayLog = logList.join('\n');
      ...
      <Card
        ...
        extra={
          <Space size={16}>
            <span
              onClick={() => {
                downloadLog(displayLog, `subtask_${subtask?.id}.log`);
              }}
              className="pointable"
            >
              <Space size={4}>
